...
vec4 createBorders(vec4 effectivebackcolour){
    ...
    if (v_backBorderWidth==0.0){
      BorderColor=effectivebackcolour;//v_backBackColor;
    }
...
vec4 getBackColour()
{
	...
	//vec4  contentColor = vec4(uv,1.0,v_backCoreColor.a); //vec4(1.0,0.0,1.0,1.0); //vec4(uv, 0.5 + 0.5*sin(iGlobalTime), 1.0);
  	...
  	vec4 effectivebackcolour = v_backBackColor;
  	 	...
  	 	//(in the distance future we might support gradients before this - in either case v_backBackColor in this equation
  		...
  		effectivebackcolour = (backdiffuse*backdiffuse.a)+(v_backBackColor*(1-backdiffuse.a));
  		 ...
  		 vec4 resultColor = createBorders(effectivebackcolour);
    #else
...
//width
   ...
   // vec2 duv = dscale * (dFdx(vTexCoord) + dFdy(vTexCoord)); //<---------------correct formula (works fine desktop)
    ...
    if (v_shadowColour.a>0.0){
    	...
    	float xo = effective_vTexCoord.x -(v_shadowXDisplacement * pixel_step.x);//v_shadowXDisplacement;
    	float yo = effective_vTexCoord.y -(v_shadowYDisplacement * pixel_step.y);//;
...
vec4 getBCFilteredCol(vec4 incoming) {
   ...
   vec4 result = incoming;
    ...
    result.rgb = result.rgb / result.a;
  	...
  	//adjust contrast
    result.rgb = ((result.rgb-1)*max(u_filterContrast,0));
...
void main() {
	...
	//effective result colours (purely as they might be set to transparent)
     ...
     effective_glowColor = v_glowColor;
	...
	if 
	    ...
	    effective_outColor  != vec4(0.0,0.0,0.0,0.0)  || 
	    effective_glowColor != vec4(0.0,0.0,0.0,0.0)  ||  
	    v_shadowColour      != vec4(0.0,0.0,0.0,0.0)
	    ) 
	{
	...
	//note; OR has a image
	#ifdef hasBackgroundImage
  	    v_backColor = getBackColour();		//if backimage, no need for a if test
  	
  	#else	
  	  	
	if ( v_backBackColor!= vec4(0.0,0.0,0.0,0.0) || v_backCoreColor!= vec4(0.0,0.0,0.0,0.0)  ){ //else we test for visible borders
